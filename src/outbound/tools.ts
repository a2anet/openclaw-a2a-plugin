...
import { A2AAgents, A2ASession, A2ATools, JSONTaskStore, LocalFileStore } from "@a2anet/a2a-utils";
...
export function createOutboundTools(params: CreateOutboundToolsParams): AgentTool[] {
    const agents = new A2AAgents(params.agents as Record<string, Record<string, unknown>>);
    const taskStore = new JSONTaskStore(`${params.stateDir}/a2a-tasks`);
    const fileStore = new LocalFileStore(`${params.stateDir}/a2a-files`);
    const session = new A2ASession(agents, { taskStore, fileStore });
    const tools = new A2ATools(session);

    return [
        {
            name: "a2a_get_agents",
            label: "A2A: Get Agents",
            description:
        ...
        },
        {
            ...
            execute: async (_id, p) => jsonResult(await tools.getAgent(p.agent_id as string)),
        },
        {
            ...
            parameters: Type.Object({
                ...
                task_id: Type.Optional(
                    Type.String({
                        description: "Attach to an existing task (for input_required flows).",
                    }),
                ),
        ...
        },
        {
            ...
            parameters: Type.Object({
                ...
                timeout: Type.Optional(
                    Type.Number({ description: "Override the monitoring timeout in seconds." }),
                ),
                poll_interval: Type.Optional(
                    Type.Number({
                        description: "Override the interval between status checks in seconds.",
                    }),
                ),
            }),
            execute: async (_id, p) =>
        ...
        },
        {
            ...
            parameters: Type.Object({
                ...
                task_id: Type.String({ description: "Task ID containing the artifact." }),
                artifact_id: Type.String({
                    description:
                        "The artifact's unique identifier (from the task's artifacts list).",
                }),
                line_start: Type.Optional(
                    Type.Number({ description: "Starting line number (1-based, inclusive)." }),
                ),
                line_end: Type.Optional(
                    Type.Number({ description: "Ending line number (1-based, inclusive)." }),
                ),
                character_start: Type.Optional(
                    Type.Number({ description: "Starting character index (0-based, inclusive)." }),
                ),
                character_end: Type.Optional(
                    Type.Number({ description: "Ending character index (0-based, exclusive)." }),
                ),
            }),
            execute: async (_id, p) =>
                jsonResult(
